...
extension EntityKind {
    ...
    /// Wave 5: dispatches via `kind.resolver` — the ONLY exhaustive switch lives in `Entity.swift:EntityKind.resolver`.
    public func resolve(pen: ResolvedPen, ctx: ResolveContext) -> ResolvedGeometry {
        resolver.resolve(pen: pen, ctx: ctx)
    }
    ...
    /// Context-aware bounding box: delegates to the registry so text/mtext/insert/xline/ray/leader
    /// use the tight font-aware / provider-aware box when `ctx` carries those providers.
    /// Wave 5: dispatches via `kind.resolver` — the ONLY exhaustive switch lives in `Entity.swift`.
    public func boundingBox(ctx: ResolveContext) -> AABB {
        resolver.boundingBox(ctx: ctx)
    }
