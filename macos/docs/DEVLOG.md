...
- [2026-06-11 16:12] 🔨 foundation-builder (general-purpose, worktree) — Phase 0.5 frozen type contract: Package→6.2/.v6 pin; CADEngine foundation types (EntityID, EntityRecord/EntityKind value model, Pen byLayer/byBlock, ResolvedGeometry + resolve() sagitta, AABB) per ADR-001/003; @MainActor @Observable CADDrawing + UndoManager value-snapshot undo (drop nonisolated(unsafe)) per ADR-002; bridge LCStatus out-param (no global error); MetalCanvasView world→clip floating-origin path rendering resolved line+circle; tests. Commit to ws/foundation. Outcome: running. Receipts: ws/foundation (pending).
