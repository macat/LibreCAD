...
**Both render paths read the RESOLVED pen** (`Pen.resolved(layer:in:)`), so per-layer linetype/width defaults flow through ByLayer. **Follow-up in flight (review SHOULD-FIX):** the stamp gate keys off record CONTENTS, so a **clone** (Copy/Array/Offset) of a source that's on layer "0" with a ByLayer pen gets wrongly re-stamped onto the active layer (AutoCAD's COPY preserves the source layer); CloneStampFix is gating the stamp on draw-vs-clone origin + adding the pinning test.
