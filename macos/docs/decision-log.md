...
## 2026-06-18 — AUTOCAD CURSOR: hide the native pointer under the drawn crosshair (two tries) — (`native-macos @ 08d62df62`, **4071 tests**, .app rebuilt)

Owner: "the default mac cursor does not disappear — i see the cross, but also the native cursor. make it like autocad." The native pointer was showing on top of the already-drawn full "spider" `CrosshairOverlayView` (default style `.full`).

- **Try 1 (FAILED) `1bcf4bf7e`:** swapped `refreshCrosshair`'s `toolCursor` from native `.crosshair` to a TRANSPARENT `blankCursor` applied via the `addCursorRect`/`resetCursorRects` machinery. Owner re-tested: **still visible.** Root cause of the failure: the canvas has transparent click-through OVERLAY SUBVIEWS (CrosshairOverlayView, gizmo, marquee/hover, UCS axis) stacked over the whole bounds; cursor rects are resolved by the FRONTMOST view, and those overlays have no cursor rect, so they SHADOW the MTKView's cursor rect and AppKit shows the default arrow. A cursor rect on the view underneath can never win (this also explains why the original native `.crosshair` rect never showed as a cross either).
- **Try 2 (WORKS) `08d62df62`:** use the layering-independent `NSCursor.hide()`/`unhide()` instead. A pure predicate `cursorShouldBeHidden(mouseInsideCanvas:crosshairVisible:windowIsKey:)` (`CanvasCursorVisibility.swift`, 8-row truth-table tested) feeds a single guarded `FlippedMTKView.reconcileCursorHidden()` — `cursorHidden` is the sole source of truth (hide only if `want && !cursorHidden`, unhide only if `cursorHidden`), so exactly one hide is ever outstanding. Reconciled from `mouseEntered`/`mouseExited`, `refreshCrosshair`, and window `didBecomeKey`/`didResignKey` (scoped to the view's window). TEARDOWN SAFETY (never leave the pointer stuck hidden): force-unhide on `viewWillMove(toWindow: nil)`, `deinit`, and `mouseExited`; window observers owned by a `nonisolated` box so the `@MainActor` view's deinit can drop them. `toolCursor` reverted to always-`nil` (arrow in select mode). Net: tool active + pointer over canvas + window key ⇒ native pointer HIDDEN, drawn crosshair is the only cursor (AutoCAD); off-canvas / select mode / app inactive ⇒ normal cursor. Removed the dead `blankCursor`. 5 predicate tests.

**LESSON:** a transparent cursor RECT is shadowed by frontmost overlay subviews — to hide the pointer under an overlay-heavy canvas use `NSCursor.hide()`/`unhide()` (balanced + teardown-safe), not a cursor rect. **Verification ceiling:** live cursor visibility is AppKit chrome the headless suite + LCShot can't observe — owner does the visual check. Minor self-correcting edge: after an app-switch the cached `mouseInsideCanvas` may be stale until the next mouse move (never stuck-hidden). Deferred/optional: hand cursor during pan; crosshair+pickbox in SELECT mode for fuller parity. Not pushed.
...
- **Root cause (confirmed with a live solver repro):** lines store INDEPENDENT endpoint coordinates with no shared-point identity; a `perpendicular` constraint pins only the ANGLE and a FREE line is parametrized about its CENTER, so the solver rotates it about the center and both endpoints drift — nothing welds the corner. The pre-existing inferred-coincident companion's tolerance (~0.01u for 10u lines) was too tight for a hand-drawn corner and only protected corners that got a constraint.
- **Lane A `5475e16f0` — AutoConstrain on draw (THE FIX).** At the `applyCommit` `.add` seam, gated on `adoptsCurrentProperties` (genuine draws only — Copy/Array/Offset/Trim/Fillet/etc. excluded) + `.line` kind, auto-adds VISIBLE (non-inferred) coincident welds for touching endpoints + at-most-one angular constraint per segment in priority H→V→perpendicular→parallel (~1° tol). Each angular is added TENTATIVELY, re-solved, and DROPPED if the component goes `.failed`/NaN or splays — so it NEVER over-constrains (the closed-rectangle case is stable). Behind `autoConstrainOnDrawKey` @AppStorage (default ON); whole draw+constraints = one ⌘Z. 14→16 tests incl. the owner's exact repro (draw 3 lines → drag corner → corners stay joined) + an OFF control + a derive-tool-adds-nothing gate lock. Critic flagged CRITICAL-1 (gate on the draw-vs-derive signal, not raw `.add`) — already satisfied; review APPROVE-WITH-NITS.
- **Lane B `c8e68292a` — 7 new AutoCAD constraint kinds.** The declared-but-unsupported kinds, end-to-end (model→solver residual→funnel→glyph→menu→⌘K→tests): geometric **collinear / concentric / equal**; dimensional **angle / diameter / horizontalDistance / verticalDistance**. Notable solver work: collinear = both line-2 endpoints' perpendicular distance to line-1's infinite line (reformulated after the angle+offset form stalled for a free far-offset line — caught via an LCShot self-render); concentric anchors arc/ellipse centers (zero added DOF); circle-center `fix` now honors `.pinStart`. Dimensional kinds LOCK the current measured value on apply. **tangent + symmetric left declared + unsupported** (critic IMPORTANT-7 split: tangent needs a distance-to-curve residual; symmetric needs a symmetry-axis `points` spec — backlog). Review APPROVE — existing 6 kinds + Lane A provably untouched, full suite green.
- **Lane D `3f3b9b375` — AutoConstrain settings.** New Preferences ▸ Constraints tab: "Auto-constrain while drawing" toggle bound to the SAME `autoConstrainOnDrawKey` + caption + read-only inferred-kinds list. (Ran parallel to B — AppSettingsView-only.)
- **Lane C `2867d383b` — Constraints GUI (the "hard to find" fix).** New `ConstraintsSidebar` (pure `ConstraintListModel` + thin views): constraints grouped Geometric/Dimensional, per-row **DELETE** (wires the previously-unreachable `removeConstraint` — one ⌘Z restores), a "Selection only" filter, and click-to-select the referenced entities; an Inspector "Constraints" section; revealed via View ▸ Show Constraints Panel (⌃⌘K) + ⌘K + the sidebar ⋯ menu. Inferred auto-companions are filtered from every list (you can't delete what you didn't author). Also completed Lane D's deferred **live-apply** (the toggle now flips already-open windows via `.lcAutoConstrainDidChange` observed in `CADCanvasView`). Review APPROVE-WITH-NITS.
- **Sequencing/why:** A solo first (it owns `CanvasModel`; fixes the bug standalone), then B (the 5-file constraint-kind critical section), then C (touches the same UI/canvas files); D parallel to B. Worktrees off `native-macos`, FF/cherry-pick merge, serial gate after each wave. **3999 → 4046 tests.** `.app` rebuilt (launch-smoke blocked only by the sandbox `open -54`, a known env limit; no launch-path change this program — owner does final GUI verify).

**Deferred (see backlog "Constraints program follow-ups"):** tangent + symmetric; inline dimensional-value edit in the panel (needs a `setConstraintValue` re-solve funnel); a snap-aperture/distance-tolerance weld for eyeballed (non-snapped) corners + polyline-endpoint welding (needs solver support); two Lane-B comment/edge NITs; one Lane-C cosmetic NIT.

Not pushed (owner pushes).

---

## 2026-06-18 — DOC SYNC + STATE AUDIT: catalog/backlog reconciled, 19 plans archived, README index, fresh roadmap — (`native-macos`, **3999 tests** green, build clean, upstream 0-behind)

Owner: "continue the project — understand where we are, run a few parallel jobs to update docs and get everything synced up." Ran a **doc-sync-audit workflow** (4 read-only investigators in parallel — feature-catalog delta, backlog delta, doc-staleness sweep, what's-next — then **14 adversarial verifiers**, one per documented P0, each tasked to REFUTE its "now done" claim). Health gate first: `swift build` clean + full serial suite **3999/3999** (420 suites, 2.2s) + `upstream/master` 0-behind + tree clean.

- **Headline finding:** the project is ~5 feature-programs past the catalog. **All 14 documented P0 must-haves are DONE end-to-end EXCEPT one — caught by the adversarial pass:** the interactive **Insert-a-block tool is PARTIAL.** Engine + DXF INSERT/MINSERT round-trip are complete, but there is **no GUI block-name picker** — `InsertTool` is inert without a `blockName` (InsertTool.swift:53,94,108-111), `CanvasModel.beginInsert(name:)` (3899-3908) has zero View-layer callers, the Insert ToolOptionsBar offers scale/rotation/MINSERT but no picker, and `BlocksSidebar.draggable(BlockDragItem)` has no canvas drop handler (dead drag). Existing blocks are placeable only via the Blocks sidebar "Insert at View Center" / "Insert Block from File…" / ⌘K (view-center, not interactive). Filed in backlog + roadmap as a high-value small wiring fix.
- **Even the catalog's 2026-06-15 "corrected" banner was itself stale:** G7 app-Preferences (`Settings{}`, ⌘,), G6a hatch boundary-arc bulge on write, G6c DXF version picker + DWG versioned save, G8 block editor/attributes/parts-library, paper-space/layouts, and UCS all shipped since. Backlog's four marquee fidelity items (hatch-bulge round-trip, MTEXT real multi-line write, DimKind ordinate/angular3p, DIMSTYLE table+writer) are all resolved in code (residual caveats: DWG-write of named DIMSTYLE is a `if(m_dwg)return;` no-op; ellipse/spline hatch boundaries still tessellated on write).
- **Docs synced (all doc-only, zero source touched):** `feature-catalog.md` banner+tail rewritten (per-row tables demoted to historical snapshot); `backlog.md` reconciled (8 items struck DONE-with-evidence + a new "Audit follow-ups (2026-06-18)" section: insert-tool picker, dead BlockDragItem drop, stale "UNWIRED" comments in InsertTool/DimStyleManagerView, multi-select edit has no multi-record test, static RelZero-Lock menu item); **19 executed one-off plans `git mv`-d into `macos/docs/archive/`**; new **`macos/docs/README.md`** doc-map (states the decision-log is authoritative; lists live vs reference vs archive); `next-features-roadmap.md` rewritten as the current ranked plan; stale banners added to `dynamic-blocks-plan.md` + `architecture-review.md`.
- **New frontier (roadmap):** the gaps are no longer table-stakes drawing features — they're (1) foreign-AutoCAD DWG/DXF fidelity **verification** (every round-trip today is self-generated; no third-party sample in repo — flagged twice), (2) a GUI-chrome screenshot tier beyond LCShot's committed-geometry ceiling, (3) the Insert-tool picker, (4) multileader CONTEXT_DATA DXF write, (5) a non-modal save-loss notice, (6) durable persistence of tables + the parametric model.
- **Next work (owner-directed, same session):** a **parametric-constraints program** — root-caused the "constrained rectangle falls apart" bug (lines store independent endpoint coords with no shared identity; `perpendicular` constrains only the angle and a free line pivots about its center, so endpoints separate; the existing inferred-coincident companion's tolerance, ~0.01u for 10u lines, is too tight to catch a hand-drawn corner) and scoped auto-constrain-on-draw + the declared-but-unimplemented AutoCAD constraint types + a constraints GUI. Program now underway — separate entry to follow when it lands.

Not pushed (owner pushes).

---

## 2026-06-18 — GUI TEST HARNESS: `LCShot` headless render-to-PNG (agent can screenshot canvas behavior) — (`native-macos`, **3999 tests**, harness smoke-rendered)

Owner: "create a test harness so you can actually test GUI … take a screenshot and analyze the results of actions … pick a good framework." Ran a research→plan→critic workflow (6 probes + planner + critic, **GO-WITH-FIXES**). **Framework chosen: a headless CoreGraphics render-to-PNG executable (`LCShot`)** driving a real `CanvasModel` from a JSON action script. Built **delegated** (one builder, staged L2→L3→L4; code-review APPROVE-WITH-NITS; nits fixed). I then **Read all 7 rendered PNGs myself** to confirm the loop works (line, mirror=original+copy, hatch fill, dimension arrows+"120" text, perpendicular re-solved to 90°, parametric center-distance, model-vs-paper).

- **Why this framework (the decisive research finding):** the repo ALREADY ships a fully headless, device-free, panel-free render path used green by `RasterExportTests` (`ExportSceneBuilder.build` → `DrawingExporter.rasterData` → `CGSceneRenderer`), and `CanvasModel` is already driven headlessly in dozens of tests. So `LCShot` is a thin executable, not a new renderer. It runs from sandboxed bash with **no window server, no TCC, no Metal, no NSApplication** — reliable turn-after-turn for an autonomous agent. `MainActor.assumeIsolated{run()}` at `main()` only (CADBench pattern; never in App.init).
- **Rejected/deferred with evidence:** real-app `screencapture`+`cliclick`/`osascript` — **confirmed non-autonomous** here (live probes: `screencapture -x` fails, `osascript` -600, `cliclick` absent, `open` -54); it's the only way to verify true SwiftUI chrome + AppKit overlays but isn't reliable unattended → deferred (would need user-granted Screen-Recording/Accessibility TCC + a launch spike). Offscreen-**Metal** — would force a risky `LineRenderer` fork and STILL miss chrome → deferred. The recommended *next* step for chrome is a per-view SwiftUI `ImageRenderer`/AppKit-bitmap snapshot (autonomous, no TCC) — also deferred.
- **Coverage ceiling (the honest tradeoff, documented loudly):** the PNG shows **committed geometry only, fit-to-page** — NOT the grid, selection, snap, in-flight preview, crosshair, grips/gizmo, the **constraint glyph**, the **live-dimension chip**, or any SwiftUI chrome. So constraint scenes prove their effect by **re-solved geometry**, not a glyph. This is the right v1 for "did action X land"; chrome/overlay verification is the deferred tier.
- **What shipped:** `LCShot` executableTarget (deps: **CADEngine only**; reaches app types via committed `_Shared*.swift` symlinks — `CADEngine ⊥ app` holds) + a JSON action DSL (`activateTool/click/value/move/commit/select/selectAll/constrain/dimConstrain/param/cmd/layout/space/zoomToFit/setOption/render`) + an `--assert` sidecar; 7 committed scenes under `macos/engine/Harness/scripts/`; `macos/scripts/lcshot.sh` wrapper; PNGs in **gitignored** `macos/build/harness-shots/`. **Hang-safety:** panel-driven tools (`image/createBlock/insert/table`) are on a deny allow-list (exit 3, never reach a modal); unsupported constraints print `WARN:` + exit 0. **Agent docs updated** (per owner follow-up "add documentation … to agents so they can use it"): `macos/docs/gui-test-harness.md` (canonical), `CLAUDE.md` Commands, and `.claude/agents/{README,coordinator,builder,acceptance-tester,standards/quality-gates}`.

---

## 2026-06-18 — DWG FIRST-CLASS: versioned `.dwg` save over libdxfrw round-3 — (`native-macos @ 71c3d85bc`, **3999 tests**, `.app` rebuilt + launch-smoked)

Owner: "yes, let's support dwg well!" Ran a research→plan→critic workflow (5 probes + planner + critic, **GO-WITH-FIXES**). The decisive research finding: **DWG OPEN + UTType + Info.plist + codec routing were ALREADY done/wired/tested** (don't redo) — the ONLY real gap was that `lc_dwg_write` threw the version away and always wrote `DRW::AC1015`, so the round-3 writers (`dwgwriter18/24/27/32`) were unreachable. Scope narrowed to **versioned DWG save**. Built **delegated** (builders + per-lane code-review + merge-by-hash + serial gate); all additive, no new EntityKind.

- **Wave 1 / Lane A** `4860831c1` (`lcdxf.cpp`/`lcdxf.h`/`DXFWriter.swift`): make versioned DWG write reachable. Added `DXFVersion.r2010=6/.r2013=7` (+ matching C `LCDxfVersion`, identical raw values appended at the end); `toDwgVersion()` maps the 5 DWG-writable tiers and **CLAMPS** R2007/R12/R14/unknown → AC1015 (the library has no `dwgwriter21`; AC1021 → `BAD_VERSION` → failed save); `lc_dwg_write` now passes `toDwgVersion(version)` to `dwg.write`. **Land-blocking invariant:** `writerVersion()` was a constant AC1015 that the version-sensitive emit gates (transparency code-440, SPLINE, DIMENSION) read — changed it to return the SAME `m_dwgVersion` passed to the write, in the same commit, so gates and bytes can't disagree. IMAGE/WIPEOUT gates (skip-on-DWG via `m_dwg!=nullptr`) left untouched. Stale "DWG is R2000-only / version ignored" doc-comments fixed.
- **Wave 2 / Lane B** `987cd0a35` (`AppSettingsView.swift`/`LibreCADDocument.swift`): `DWGExportVersion` enum (R2000/R2004/R2010/R2013/R2018 — **no R2007/R12/R14**) + a second picker in Preferences ▸ General ▸ Files + an honest always-visible caption ("Some data may not round-trip to DWG (custom layers, block contents, viewports, parametric tables). Use DXF for full fidelity."); `resolvedDWGExportVersion()` reads the key off-main and **clamps defense-in-depth** (a stored "r2007"/garbage → R2000); the `.dwg` codec branch now threads the chosen version into `writeEntities(...toDWGPath:version:)`. Default stays **R2000** (the only externally-blessed tier).
- **Wave 2 / Lane C** `b109890fb` + follow-up `6df7fb074` (test files only): 12 engine tests (per-version magic bytes AC1018/AC1024/AC1027/AC1032 + reopen round-trip + R2007/R12/R14 clamp-safety) and 20 app-layer tests (`DWGVersionPickerTests`: tier set, engine-version mapping, the clamp at every layer, and a codec **Save-As magic round-trip** proving the preference reaches the on-disk DWG header). 3967 → **3999**.

**Honest-gaps stance (the critic's call):** the per-save NSAlert was **descoped** — there is NO View-layer save-observation hook today (DocumentGroup Save runs through off-main `LibreCADDocument.fileWrapper`; a modal there hangs the headless suite), so the alert would be speculative wiring. The static picker caption fully satisfies the honest-notice goal. **Verification ceiling (flag to owner):** the in-repo round-trips are self-generated (our writer ↔ our reader) — they prove we emit + re-read each requested version, NOT foreign-AutoCAD fidelity at R2010/R2013/R2018 (needs a real third-party `.dwg` + ODA/`dwgread`); default stays R2000 until externally blessed. **Deferred:** native ACAD_TABLE / block-member geometry / viewport round-trip (writer-architecture gaps, NOT closable by a higher version); parametric constraints/parameters still ride only the in-session payload (DXF/DWG bytes can't carry them — same as before).
...
- **MLINE** (full): `EntityKind.mline` (W0, 22-file atomic arm) + DXF round-trip via **stock `writeMLine` + LIBRECAD-XDATA element offsets, ZERO vendored edits** (foreign per-element offsets unrecoverable → honest pinned fallback) + `MLineTool` + UI (justification/scale options, Draw menu, palette). Square ends, no MLEDIT/fill (deferred).
- **Parametric constraints** (MVP): additive `CADDrawing.constraints` + a pure **Levenberg–Marquardt solver** (DOF packing, fix-anchor, numeric Jacobian, `.failed` on over/under-constrained — never a partial write); a **re-solve seam** on grip/inspector edit (edit+solve = ONE undo via the explicitGroup path); selection-based **Constrain menu** (geometric {coincident/H/V/parallel/perpendicular/fix} + dimensional {distance/radius} — dimensional locks the current measured value) + a read-only glyph overlay + Show-Constraints toggle. Deferred: angle/equal/concentric/etc., interactive pick tools, driven-value editing, constraint DXF persistence.
- **Tables** (MVP): additive `CADDrawing.tables` + `TableGeometry` materializer (grid + cell text via the shared TextShaper, merge-span handling) + on-canvas render + `TableTool` (places a default 3×3) + **DXF persist = explode to LINE+TEXT on save** (libdxfrw can't carry ACAD_TABLE → reopened as loose geometry, NOT a re-editable table — pinned caveat). Deferred: cell-text edit UI, table styles, true round-trip.
- **Fields** (MVP): additive `TextData/MTextData.fields` + `FieldEvaluator`/`FieldContext` (date/layout/file tokens, "####" on miss) + resolve-time substitution (nil-context byte-identical) + live render on canvas + Inspector + menu/palette insert. Deferred: object-property fields, fileName value (doc URL not reachable from the model), cursor-position insert.
- **Isometric** (full MVP): `IsoPlane` (3 planes) + iso snap/grid (nil = rectangular byte-identical) + iso-ortho + iso crosshair + iso-circle (ellipse ratio tan30°) + ISO status chip + **F5** plane-cycle + View menu + options-bar Isocircle + `$SNAPSTYLE` round-trip (zero vendored edit). Deferred: per-VPORT plane, DIMISO, UCS↔iso coupling.

**Dispatch:** ~25 builder/reviewer agents in pre-created `.claude/worktrees/` worktrees, foreground-concurrent within a wave, EntityKind serialized first, the three CADDrawing-sharing engine lanes (fields→tables→iso) sequenced, the wire-wave split into 3 sequential sub-waves by feature (hot-file contention). Reviews: APPROVE/APPROVE-WITH-NITS across the lot. **Note (backlog NIT):** the tables lane used XCTest where the project standard is swift-testing — convert in a polish pass. **`.app` rebuilt + launch-smoked** (no SIGTRAP; the four features menu/palette-reachable).
